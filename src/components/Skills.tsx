...
export default function Skills() {
    ...
    return (
        <div className={`max-w-4xl mx-auto px-6 py-16 flex flex-col items-center text-center transition-all duration-700 ease-out transform ${isLoaded ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight inline-block relative pb-3 mb-6">
                Skills & Certificates
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-2xl text-left mb-6">
                <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm flex flex-col justify-between hover:border-blue-300 transition-colors duration-300">
                    <div>
                        <h3 className="text-slate-900 font-bold text-lg mb-3 flex items-center gap-2.5">
                            <img src={backendImage} alt="Backend icon" className="w-5 h-5 object-contain" />
                            Backend & Full-Stack
                        </h3>
                        <p className="text-slate-600 text-sm">Python, C++, Node.js, React, Vue.js, PostgreSQL, Prisma.</p>
                    </div>
                </div>
                <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm flex flex-col justify-between hover:border-emerald-300 transition-colors duration-300">
                    <div>
                        <h3 className="text-slate-900 font-bold text-lg mb-3 flex items-center gap-2.5">
                            <img src={computerImage} alt="AI icon" className="w-5 h-5 object-contain" />
                            AI & Machine Learning
                        </h3>
                        <p className="text-slate-600 text-sm">LLMs, Algorithms, Scikit-Learn, Recommendation Pipelines.</p>
                    </div>
                </div>
            </div>

            <div className="w-full max-w-2xl text-left mb-10">
                <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm flex flex-col justify-between hover:border-indigo-300 transition-colors duration-300">
                    <div>
                        <h3 className="text-slate-900 font-bold text-lg mb-3 flex items-center gap-2.5">
                            <img src={certificateImage} alt="Certificate icon" className="w-5 h-5 object-contain" />
                            Certificates
                        </h3>
                        <p className="text-slate-600 text-sm italic">Currently learning!</p>
                    </div>
                </div>
            </div>
